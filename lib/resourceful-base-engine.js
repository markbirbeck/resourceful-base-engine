...
var BaseEngine = exports.BaseEngine = function BaseEngine(config) {
  ...
  _.extend(this.supports, config.supports);
 ...
 * but also possibly with a document to store or search for:
 */
...
BaseEngine.prototype.destroy = function(id, callback) {
  return this.request('del', id, null, function(err, res) {
    if (err || (res && res.status && res.status !== 204)){
      callback(err, { 'status': res.status });
    } else {
      callback(
        null
      , _.extend({ 'status': 204 }, (typeof res === 'string') ? {id: res} : res)
      );
    }
  });
};
...
BaseEngine.prototype.find = function(conditions, callback) {
  return this.request('find', conditions, function(err, res){
    if (err){
      callback(err);
    } else {
      callback(null , res);
